...
def use_case(
    ...
    return UnlockCartUseCase(uow=uow, distributed_lock_system=distributed_lock_system)
...
async def test_cart_already_locked_for_updates(
    ...
    async with uow(autocommit=True):
        cart_in_db = await uow.carts.retrieve(cart_id=cart.id)

    assert cart.status == cart_in_db.status
