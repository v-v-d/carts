...
from app.app_layer.interfaces.use_cases.carts.dto import CartDeleteInputDTO
...
@pytest.fixture()
def dto(cart: Cart, cart_item: CartItem, auth_data: str) -> CartDeleteInputDTO:
    return CartDeleteInputDTO(auth_data=auth_data, cart_id=cart.id)
...
async def test_ok(
    ...
    use_case: CartDeleteUseCase,
    dto: CartDeleteInputDTO,
    cart: Cart,
    uow: TestUow,
) -> None:
    ...
    assert result.status == cart.status == CartStatusEnum.DEACTIVATED
...
async def test_deactivate_by_admin(
    redis: AsyncMock,
    redis_lock_config: RedisLockConfig,
    use_case: CartDeleteUseCase,
    cart: Cart,
