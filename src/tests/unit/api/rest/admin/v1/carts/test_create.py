...
def use_case(request: SubRequest, mocker: MockerFixture) -> AsyncMock:
    ...
    if "returns" in request.param:
        mock.create_by_user_id.return_value = request.param["returns"]
    elif "raises" in request.param:
        mock.create_by_user_id.side_effect = request.param["raises"]
...
async def test_ok(http_client: AsyncClient, use_case: AsyncMock, url_path: str) -> None:
    ...
    assert response.json() == {
        "created_at": use_case.create_by_user_id.return_value.created_at.isoformat(),
        "id": str(use_case.create_by_user_id.return_value.id),
        "user_id": use_case.create_by_user_id.return_value.user_id,
        "status": use_case.create_by_user_id.return_value.status.value,
        "items": use_case.create_by_user_id.return_value.items,
        "items_qty": float(use_case.create_by_user_id.return_value.items_qty),
        "cost": float(use_case.create_by_user_id.return_value.cost),
        "checkout_enabled": use_case.create_by_user_id.return_value.checkout_enabled,
        "coupon": use_case.create_by_user_id.return_value.coupon,
    }
